...
#   Test Model
    ...
    for x in testX:  
        exper.append(clf.predict(np.array([x])))
    for y in testY:
        act.append(y)
    ...
    predictions = []
    predictions.append(clf.predict(np.array(Xback))[0])
...
#   train("MMMBB",500,1,2,16,1,1700,1,randL[0:int(len(randL)*.3)],True)
        a = train("LUVBBF3",window,1,2,16,1,1000-window,z,daysBackward,randL[0:int(len(randL)*.3)],True)
##
        predict.append(a[3])
        print predict
        error.append(a[2])
